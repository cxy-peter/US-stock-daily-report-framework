...
from .provider_registry import AcceptedCloseBatch
...
class LedgerValuation:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "prices", MappingProxyType(dict(sorted(self.prices.items()))))
...
class PortfolioLedger:
    ...
    def settle_modeled_dca_batch(
        ...
        for symbol, amount in plan.base_amounts.items():
            ...
            if close.expected_session != session:
                raise LedgerSettlementBlocked(f"{symbol} close has the wrong session")
            if (
                close.status != "accepted"
                or close.finality != "confirmed"
                or close.independent_source_count < 2
                or close.selected_observation_id is None
                or not close.eligible_for_ledger_input
            ):
                raise LedgerSettlementBlocked(f"{symbol} close is not eligible for atomic ledger input")
            if close.currency != self.policy.currency or close.selected_price is None:
                raise LedgerSettlementBlocked(f"{symbol} close has no eligible ledger price")
        ...
        with self._transaction() as connection:
            ...
            if plan.funding_mode == "modeled_external_contribution":
                contribution = self._append_event(
                    connection,
                ...
                contribution_id = contribution.event_id
            fill_ids: list[str] = []
    ...
    def record_valuation(
        ...
        with self._transaction() as connection:
            ...
            for symbol in sorted(required_symbols):
                close = closes[symbol]
                if close.expected_session != session:
                    raise LedgerSettlementBlocked(f"{symbol} valuation close has the wrong session")
                if not close.valuation_permitted or close.selected_price is None:
                    raise LedgerSettlementBlocked(f"{symbol} has no permitted valuation close")
                if close.finality not in {"confirmed", "confirmed_with_warning"}:
                    raise LedgerSettlementBlocked(f"{symbol} valuation close is not final")
                if close.currency != self.policy.currency:
                    raise LedgerSettlementBlocked(f"{symbol} valuation currency is inconsistent")
            ...
            previous_payload = None if previous is None else json.loads(previous["payload_json"])
            ...
            if previous_payload is None:
                ...
                previous_nav = _decimal(previous_payload["nav"], "previous nav", minimum=ZERO)
                daily_pnl = nav - previous_nav - net_external_flow
                denominator = previous_nav + weighted_external_flow
                if denominator <= ZERO:
                    raise LedgerProjectionError("weighted starting capital must be positive")
                daily_return = daily_pnl / denominator
                raw_prior_twr = previous_payload.get("cumulative_twr")
                if raw_prior_twr is None:
                    ...
                    prior_twr = _decimal(raw_prior_twr, "prior cumulative TWR")
                    cumulative_twr = (ONE + prior_twr) * (ONE + daily_return) - ONE
            ...
            return self._valuation_from_row(row)
    ...
    def _dca_result_from_connection(
        ...
        *,
        ...
        contribution_id: str | None = None
        for row in rows:
            child_payload = json.loads(row["payload_json"])
            if child_payload.get("batch_key") != batch_key:
                continue
            if row["event_type"] == "modeled_dca_fill":
                fill_ids.append(str(row["event_id"]))
            else:
                contribution_id = str(row["event_id"])
        return DcaSettlementResult(
            status="settled",
            session=_date(marker["session_date"]),
            ...
            batch_event_id=str(marker["event_id"]),
            fill_event_ids=tuple(fill_ids),
            contribution_event_id=contribution_id,
            total_configured_amount=_decimal(payload["total_configured_amount"], "total configured"),
            total_spend=_decimal(payload["total_spend"], "total spend"),
            total_residual=_decimal(payload["total_residual"], "total residual"),
    ...
    def _valuation_rows(
        ...
        return [
            row
            for row in rows
            if json.loads(row["payload_json"]).get("book_kind") == book_kind
        ]
    ...
    def _valuation_from_row(
        ...
        *,
        ...
        payload = json.loads(row["payload_json"])
        prices = {
            _symbol(symbol): _decimal(value, f"valuation price {symbol}", positive=True)
            for symbol, value in payload["prices"].items()
        }
        return LedgerValuation(
            valuation_event_id=str(row["event_id"]),
            book_kind=str(payload["book_kind"]),
            session=_date(row["session_date"]),
            accepted_close_batch_id=str(payload["accepted_close_batch_id"]),
            currency=str(payload["currency"]),
            cash=_decimal(payload["cash"], "valuation cash"),
            securities_value=_decimal(payload["securities_value"], "securities value"),
            nav=_decimal(payload["nav"], "valuation NAV"),
            prices=prices,
            daily_pnl=(
                None
            ...
            net_external_flow=_decimal(payload["net_external_flow"], "net external flow"),
            weighted_external_flow=_decimal(
                payload["weighted_external_flow"], "weighted external flow"
            ),
            idempotent_replay=idempotent_replay,
        )
