...
def test_policy_brief_extracts_complete_claims_not_mention_counts():
    ...
    claim = next(
        item for item in result.top_claims if item.topic == "trade_tariff"
    )
...
def test_actor_authority_and_media_disagreement_affect_importance():
    ...
    result = build_political_brief(
        ACTORS, documents, media_assessments=media, as_of=NOW
    )
    trump_claim = next(
        claim
        for claim in result.top_claims
        if claim.actor_id == "donald_trump" and claim.topic == "ai_semiconductor"
    )
    press_claim = next(
        claim
        for claim in result.top_claims
        if claim.actor_id == "press_secretary" and claim.topic == "ai_semiconductor"
    )
    assert trump_claim.importance > press_claim.importance
    assert trump_claim.media_disagreement is not None
    assert trump_claim.media_disagreement > 0.5
...
class _Response:
    def __init__(self, text="", payload=None):
        self.text = text
        self.content = (
            text.encode("utf-8") if payload is None else json.dumps(payload).encode()
        )
...
def test_white_house_collector_reads_public_listing_and_article():
    ...
    result = collector.collect(
        listing_urls=["https://www.whitehouse.gov/presidential-actions/"],
...
def test_x_collector_redacts_token_and_returns_official_posts():
    ...
    collector = XOfficialCollector(
        bearer_token="super-secret", session=session, clock=lambda: NOW
    )
    result = collector.collect(
        {"donald_trump": "POTUS"}, start_time=NOW - dt.timedelta(days=1)
    )
...
def test_rss_media_collector_preserves_media_as_separate_source():
    ...
    collector = RssMediaCollector(
        session=_QueueSession([_Response(rss)]), clock=lambda: NOW
    )
    result = collector.collect(
        [
            {
...
def _live_snapshot(
    spread: float = 0.02, liquidity: float = 2_000_000
) -> LiveMarketSnapshot:
    history = tuple(
...
def test_live_polymarket_uses_velocity_liquidity_and_spread():
    ...
    poor = score_live_market(
        _live_snapshot(spread=0.18, liquidity=100), calibration_multiplier=0.2
    )
...
def test_polymarket_public_client_has_no_order_methods_and_parses_public_data():
    ...
    assert not (
        {"place_order", "create_order", "cancel_order", "post_order"}
        & method_names
    )
